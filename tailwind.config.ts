    ...
    extend: {
      colors: {
        ...
        */
        ...
        /*
          나머지 canonical 색. GREEN·LINE 과 같은 이유로 토큰이다 — 값이 흩어져 있으면
          바꿀 수 없고, 규칙을 grep 으로 확인할 수도 없다. #2C3930 하나가 67개 파일에
          있었으니 INK 를 조정하는 건 166곳 치환이었다(= 사실상 "못 바꾼다").

          ⚠️ 골드(#E7BD5B·#DDBF68)는 일부러 토큰으로 만들지 않았다. 카카오 로그인 버튼이
          카카오 브랜드 노랑(#FFEC9A)을 쓰는데, 우리 골드 토큰이 있으면 그 자리로 잘못
          흘러간다. 외부 브랜드색과 우리 액센트는 섞지 않는다.
        */
        ink: {
          DEFAULT: '#2C3930', // INK — 주 텍스트·아이콘 (크림 위 11.8:1)
          muted: '#60655C', // INK-muted — 보조·캡션 (크림 위 5.8:1)
          disabled: '#B4B4B4', // 비활성. 의도적 de-emphasis 라 대비 기준을 적용하지 않는다
        },
        cream: {
          DEFAULT: '#FFFCEF', // 페이지 배경. 다크 버튼 위 반전 텍스트도 이 값이다
          sub: '#F6F0D7', // 베이지 서브 — 구분 면
          /*
            한 단 더 눌린 크림. **사진이 오기 전 자리**(PhotoPost·FavoriteGrid)와 **눌림/호버
            면**(TimelinePage)이 쓴다 — 둘 다 "크림보다 한 단 들어간 면" 이라 같은 값이다.
            ⚠️ `sub`(#F6F0D7)로 접지 않는다. sub 는 노랑이 남은 구분 면이고 이건 회색이
            섞여 사진 자리에서 바탕처럼 물러난다. 실제로 3개 파일이 이 값으로 합의돼 있었다.
          */
          deep: '#EDE7D2',
        },
        error: {
          DEFAULT: '#DC2626', // 에러 텍스트·삭제 버튼
          surface: '#FEF7F7', // 경고 배너 바탕. ⚠️ 더 진한 분홍은 못 쓴다 (§1.1 참조)
        },
        bark: '#7A5C3A', // 동선 경로선. 선·데코 전용이고 텍스트로는 쓰지 않는다
        /*
          390px 컬럼 **바깥** 배경. 데스크톱에서만 보이고 `AppShell` 한 곳이 쓴다.
          canonical 에는 처음부터 있었는데 토큰만 없어서 lint 가 못 지키던 자리다
          (쓰는 곳이 하나뿐이어도 토큰이어야 규칙을 grep 으로 확인할 수 있다).
          ⚠️ 앱 **안**에는 쓰지 않는다 — 크림 위에 얹으면 면이 두 겹으로 읽힌다.
        */
        shell: '#EFEDE7',
      },
