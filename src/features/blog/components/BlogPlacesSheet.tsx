  ...
  return createPortal(
    <div className="fixed inset-0 z-[60] bg-black/50" role="presentation" onClick={onClose}>
      ...
      >
        ...
        <h2 id="blog-places-title" className="text-[17px] font-bold text-[#20251f]">
          방문한 장소 <span className="text-[#7f9648]">{sections.length}곳</span>
        </h2>
        ...
        <ul className="mt-3 max-h-[64vh] overflow-y-auto">
          {sections.map((section) => (
            <li key={section.treeId} className="flex items-center gap-3 border-b border-[#ececdf] py-3 last:border-0">
              ...
              <p className="min-w-0 flex-1 truncate text-[15px] font-bold text-[#20251f]">{section.heading}</p>
            </li>
          ))}
        </ul>
      </section>
