...
export function AuthShell({ children }: PropsWithChildren) {
  return (
    <main className="relative flex min-h-full w-full flex-col bg-[#FFFCEF] px-6 pb-[max(calc(env(safe-area-inset-bottom)+1.25rem),2.5rem)] pt-[calc(env(safe-area-inset-top)+0.75rem)] text-[#263122]">
      {children}
    </main>
  );
}
