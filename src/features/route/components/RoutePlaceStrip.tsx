...
/**
 * 접었을 때 남는 높이. 손잡이 + 따라가기 줄(없으면 `전체 동선` 줄) + `다음` + 안전영역의
 * 어림값이다.
 *
 * **지도 패딩에만 쓰는 값이라 어림으로 충분하다** — 레이아웃은 여전히 내용이 정한다.
 * 접힌 시트는 화면을 조금만 먹으므로 몇 px 어긋나도 마커가 가려지지 않는다.
 * (저장된 동선 보기에는 `다음` 이 없어 실제로는 이보다 낮다 — 지도 아래가 조금 더 빌 뿐이다.)
 */
export const SHEET_COLLAPSED_PX = 159;
 ...
 *
 * 끌기를 **위아래 끝(손잡이·`다음` 줄)에만** 붙인다 — 가운데 장소 목록도 세로로 움직이는
 ...
 *
 * 목록만 굴러간다 — 날짜 필터와 `다음` 줄은 위아래에 붙박이라 굴리는 내내 제자리에 있다.
 *
    ...
    */
    ...
    >
      ...
      {/* 동선 따라가기. 접히는 영역 **밖**이라 접어도 남는다.
          ...
          밀리면 시트가 접히면서 장소까지 넘어간다. 시트를 잡는 자리는 손잡이와 `다음` 줄이다. */}
      {stepper}
      ...
      {/*
        시트 맨 아래에 붙박인 `다음`. **접히든 펴지든 자리가 안 바뀐다** — 접을 때 버튼이
        위아래로 뛰어다니면 어디를 누르려던 건지 놓친다. 가운데(필터·개수·목록)만 여닫히고
        이 줄은 그 아래에 그대로 있다.

        **한동안 `동선저장` 이었다.** 이름 짓기가 시트에서 화면으로 승격하면서(③단계) 이
        버튼은 저장하지 않고 넘기기만 한다 — 되돌릴 수 없는 동작은 이제 그 화면에 있다.

        모양은 ①단계(날짜 고르기)의 `다음` 과 같은 것을 쓴다(`PrimaryCta`). 같은 흐름의
        세 화면이 높이 52/48, 라운드 24/12, 바닥색 초록/먹색으로 서로 달랐다.
      ...
      */}
      {onNext && (
        <div className="shrink-0 touch-none px-5 pt-3" {...drag.handlers}>
          <PrimaryCta
            onClick={() => {
              // 끌어서 시트를 여닫은 손짓이면 넘어가기까지 하면 안 된다 — 끌기로 판정된 뒤에
