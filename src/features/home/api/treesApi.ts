 ...
 * 나무(지도) API 레이어.
 *
 * ⚠️ **여기에 목 폴백은 없다 — 이 레이어는 실 API 만 말한다.** 한때 이 파일이 토큰이
 * 없거나 호출이 실패하면 목데이터를 돌려줬는데, 그 함수를 쓰는 곳이면 어디로든 가짜
 * `treeId` 가 흘러들었다(`POST /routes` 로 새면 400). 폴백은 그것이 필요한 화면 하나가
 * 자기 사정으로 갖는다 → `hooks/useTrees.ts`.
 */
