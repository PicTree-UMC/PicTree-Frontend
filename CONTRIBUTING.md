...
**`main` + 단명 기능 브랜치 2단계 구조를 씁니다(GitHub Flow).** `dev`/`release` 같은 중간 통합 브랜치는 두지 않습니다.

- **배포가 `main` 하나에 물려 있습니다.** Netlify가 `main` 머지 시점에 바로 빌드·배포합니다(`netlify.toml`). `dev`를 추가하면 그 앞에 배포 대상이 없는 채로 병합 단계만 하나 늘어나, 오히려 "지금 뭐가 배포됐는지"를 `main`과 `dev` 두 곳에서 따로 확인해야 하는 부담이 생깁니다.
- **팀 규모와 릴리즈 주기가 dev를 필요로 하지 않습니다.** 버전을 나눠 배포하거나 여러 기능을 한 번에 묶어 릴리즈하는 주기가 없고, 기능 브랜치가 PR 승인 즉시 `main`에 들어가는 흐름이라 중간에 모아둘 이유가 없습니다.
- **대신 모든 병합은 PR + 리뷰를 거칩니다.** dev 없이 바로 배포되는 만큼, 리뷰 없는 병합이 사고로 이어지지 않는 게 중요합니다. ⚠️ 지금은 관행으로만 지켜지고 있고 GitHub 브랜치 보호 규칙(리뷰 승인 필수)은 아직 `main`에 걸려 있지 않습니다 — 관행이 강제력으로 바뀌어야 이 전략의 안전장치 근거가 완전해집니다.
