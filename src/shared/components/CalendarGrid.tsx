...
/*
  ...
  ⚠️ **되돌리지 말 것 — 대비 미달(일 2.36:1)은 알고 유지하는 것이다** (이슈 #58, 판정 2026-08-10).
  4.5:1 로 올리려면 일요일을 #D94F4F 급으로 내려야 하는데 그러면 ERROR(#DC2626)와 구분이 안 돼
  **일요일이 에러로 읽힌다.** 토요일도 짝을 맞춰 어두워져야 해서 관례색이 통째로 무너진다.
  성립 근거는 "색이 정보를 혼자 나르지 않는다" 하나다 — 첫 열이 일요일이고 숫자는 INK 로 그린다.
  근거 전문은 docs/design-guidelines.md §1.2.
*/
const SUNDAY = '#FF8080';
