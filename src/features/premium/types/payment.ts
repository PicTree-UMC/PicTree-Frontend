...
/**
 * `POST /payments/{id}/cancel` 요청.
 *
 * ⚠️ `cancelReason` 은 **필수**다(서버 `@IsNotEmpty`, 200자 제한). 화면에서 사유를 받지
 * 않고 고정 문구를 보낸다 — 이 앱에서 취소를 누르는 길이 하나뿐이라 사유가 항상 같고,
 * 자유 입력을 받으면 취소 한 번에 글쓰기를 시키는 꼴이 된다. 관리자 취소·부분 취소가
 * 생기면 그때 사유가 갈리므로 여기 필드를 열면 된다.
 */
...
/**
 * 취소 결과. **목록 항목(`PaymentDto`)과 모양이 다르다** — 서버가 취소 전용 DTO 를 준다.
 * 그래서 이 값을 캐시에 직접 밀어 넣지 않고 무효화로 다시 받는다(`useCancelPayment`).
 */
 ...
 * 취소할 수 있는 결제인가.
 *
 * 서버 판정(`validateCancelablePayment`)과 같은 조건이다 — `DONE` 이면서 결제사 식별자가
 * 있어야 한다. 어긋나면 `PAYMENT409 취소할 수 없는 결제 상태입니다.` 가 온다.
 *
 * ⚠️ **화면이 먼저 거르는 이유는 버튼을 숨기기 위해서다.** 서버가 어차피 막지만, 눌러 본
 * 뒤에 안 된다고 말하는 것과 처음부터 없는 것은 다르다. 판정을 두 곳에 두는 대신
 * **서버 조건을 그대로 옮겨 적고** 여기 주석으로 출처를 묶어 둔다.
 */
export const canCancelPayment = (payment: PaymentDto): boolean =>
  payment.status === PAYMENT_STATUS.done && !!payment.providerPaymentId;
