    ...
    <ModalShell bottom onClose={onCancel}>
      ...
      <div className="mt-4 overflow-hidden rounded-xl border-2 border-[#c5d89d] bg-white text-[13px]">
        ...
        */}
        ...
        <div className="flex justify-between border-t border-[#ecf6d8] px-7 py-3"><span>결제 금액</span><span>{details.price}</span></div>
      </div>
      <p className="mt-4 text-center text-[10px] leading-5">매월 자동으로 갱신돼요<br/>다음 결제일 전까지 언제든 해지할 수 있어요</p>
      <button className="mt-7 h-[62px] w-full rounded-xl bg-[#788f4a] font-bold text-white shadow-lg" onClick={onPay}>결제하기</button>
      <button className="mt-3 h-[45px] w-full rounded-xl bg-[#ecf6d8] font-bold text-white" onClick={onCancel}>취소</button>
    </ModalShell>
  );
}
