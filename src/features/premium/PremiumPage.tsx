...
function PremiumNavBar() {
  // 히스토리가 없을 때(주소 직접 입력, PWA 첫 화면, 토스 결제 착지 후)는 마이페이지 메인
  // 탭으로 — 그 탭의 **요금제 타일**이 이 화면의 주 입구라 거기가 부모 자리다.
...
/**
 * 요금제·결제 화면. **혼자 서는 페이지다** — 뒤로가기를 달고 제 배경을 깐다.
 *
 * 들어오는 길 셋: 마이페이지 메인 탭의 **요금제 타일**(주 입구) · 블로그의 작성 제한과
 * 업그레이드 버튼 · 토스 결제 착지 후 복귀.
 *
 * ⚠️ 한때 마이페이지 안에 '플랜 탭' 으로 끼워 넣는 안을 짓다가 되돌렸다. 이 화면은 앱에서
 * 유일하게 크림이 아닌 바닥(그라데이션)을 쓰는데, 탭 하나만 배경이 다르면 탭 줄이 화면을
 * 가르는 게 아니라 두 앱을 오가는 것처럼 읽힌다. 게다가 `fixed inset-0` 배경이라 위쪽
 * 머리글·탭 줄이 그라데이션의 제일 진한 구간을 가려, 탭 안에서는 제 모습이 나오지도 않았다.
 */
