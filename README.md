...
로그인은 **카카오 · 구글 소셜 로그인만** 지원합니다(이메일/비밀번호 없음). 아래 순서로 핵심 흐름을 확인할 수 있습니다.

1. 배포 주소 접속 → 카카오 또는 구글 계정으로 로그인
2. 지도 홈에서 사진 업로드 → 위치 정보 추출 → 장소 기록 생성
3. 타임라인에서 생성한 기록 확인
4. 동선(`/journey`)에서 날짜를 선택해 동선 저장
5. 마이페이지에서 캘린더 · 즐겨찾기 · 구독/결제 확인
...
`src/features/*/api/` 아래 API 계층 함수를 전수 조사한 기준입니다(2026-08-12). 화면 자체는 8개 feature 전부 구현되어 있고, 아래는 **데이터가 실제 서버와 연동되는지**를 기능별로 구분한 것입니다.

| 기능 | 상태 | 비고 |
| --- | --- | --- |
| 회원가입 / 로그인 | 실 API 연동 | 카카오·구글 소셜 로그인, 토큰 재발급 |
| 지도 홈 | 실 API 연동 | 미인증/개발 환경일 때만 지도 마커 4개가 로컬 목데이터로 대체됨(프로덕션 빌드에서는 비활성) |
| 기록 생성 (카메라) | 실 API 연동 | 사진 업로드 포함 |
| 타임라인 | 실 API 연동 | |
| 동선 | 실 API 연동 | |
| AI 블로그 작성 | 실 API 연동 | |
| 마이페이지 (구독/결제/캘린더/즐겨찾기) | 실 API 연동 | 토스페이먼츠 정기결제(빌링키) 포함 |

**API 계층 함수 43개 중 39개(약 90%)가 실 백엔드에 연동**되어 있고, 나머지는 개발 환경 전용 지도 폴백 1개와 서버 호출이 필요 없는 순수 변환 함수(타임라인·동선 후보 가공) 3개입니다. Mock 또는 로컬 스텁으로 남아 있는 화면은 없습니다.
...
│   ├── route/           # 동선 조회 및 저장 (URL 은 /journey 유지)
│   ├── blog/            # AI 블로그 작성
│   └── profile/         # 구독, 결제, 캘린더, 즐겨찾기, 알림 설정
├── router/              # 라우터 매핑
